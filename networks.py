...
class SoftMaskProxyNet(nn.Module):
    def __init__(self, num_features, adaptive=True, fixed_weights=None, temp_scale=5.0): 
        super().__init__()
    ...
    def get_mask(self, m1, m2, m3):
        w = F.softmax(self.w_logits / self.temp_scale, dim=0) if self.adaptive else self.fixed_w
...
def train_softmask_dnn_proxy(pop1, pop2, pop3, xtrain, ytrain, num_features, adaptive=True, fixed_weights=None, temp_scale=5.0):
    ...
    model = SoftMaskProxyNet(num_features, adaptive, fixed_weights, temp_scale).to(device)
    ...
    for _ in range(800):
        ...
        if adaptive:
            with torch.no_grad():
                current_w = F.softmax(model.w_logits / temp_scale, dim=0).cpu().numpy()
                weight_history.append(current_w.tolist())
        loss_history.append(epoch_loss / len(loader))
            
    with torch.no_grad():
        final_lambda = F.softmax(model.w_logits / temp_scale, dim=0).cpu().numpy() if adaptive else None
    ...
    return mask.cpu().numpy(), final_lambda, weight_history, loss_history

# ---------------------------------------------------------
# Exp 1: Multi-Class Proxy (SoftMaskProxyNetMulti)
# ---------------------------------------------------------
class SoftMaskProxyNetMulti(nn.Module):
    def __init__(self, num_features, num_classes, adaptive=True, fixed_weights=None, temp_scale=5.0): 
        super().__init__()
        self.adaptive = adaptive
        self.temp_scale = temp_scale 
...
def train_softmask_dnn_proxy_multi(pop1, pop2, pop3, xtrain, ytrain, num_features, num_classes, adaptive=True, fixed_weights=None, temp_scale=5.0): 
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    model = SoftMaskProxyNetMulti(num_features, num_classes, adaptive, fixed_weights, temp_scale).to(device)
...
def train_softmask_dnn_dual_proxy(pop1, pop2, pop3, xtrain, yc_train, yr_train, num_features, adaptive=True, fixed_weights=None, temp_scale=5.0):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    model = DualProxyMaskNet(num_features, adaptive, fixed_weights, temp_scale).to(device)
